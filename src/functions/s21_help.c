...
int l_shift(s21_decimal *value) {
  ...
  int result = 0;
  for (int i = 0; i < 3; i++)
    b[i] = is_bit_set(value->bits[i], 31);
  if (b[2] == 0) {
    for (int i = 0; i < 3; i++)
      value->bits[i] <<= 1;
    for (int i = 0; i < 2; i++)
...
int left_shift(s21_decimal *number, int shift) {
  int result = 0;
  for (int i = 0; i < shift && !result; i++)
    result = l_shift(number);
  return result;
}
...
void r_shift(decimal_overflow *value) { // деление на 2
  short b[5];
  for (int i = 1; i < 5; i++)
    b[i] = is_bit_set(value->bits[i], 0);
  for (int i = 0; i < 6; i++)
    value->bits[i] >>= 1;
  for (int i = 4; i > 0; i--)
    if (b[i])
      value->bits[i - 1] = set_bit(value->bits[i - 1], 31);
}
